...
dir.create("///groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/probabilityOfFindingTC/zygoticTranscripts_vectorized_noMultimappers/numberOfGenes_zygotic/")
...
zygoticReads_samples = list.files(path = "//groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/probabilityOfFindingTC/zygoticTranscripts_vectorized_noMultimappers/",pattern = "freqAllMutations_zygoticTranscripts*")
#zygoticReads_samples = zygoticReads_samples[28]
zygoticReads_samples_path = paste0("//groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/probabilityOfFindingTC/zygoticTranscripts_vectorized_noMultimappers/",zygoticReads_samples)
...
for(i in 1:length(zygoticReads_samples_data)){
  ...
  save(allMutations,file =paste0("//groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/probabilityOfFindingTC/zygoticTranscripts_vectorized_noMultimappers/numberOfGenes_zygotic/" ,zygoticReads_samples[i],".Rdata"))
  
}



allRdata = list.files("/Volumes/groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/probabilityOfFindingTC/zygoticTranscripts_vectorized_noMultimappers//numberOfGenes_zygotic//",pattern = "*.Rdata")
allRdata_path = paste0("/Volumes/groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/probabilityOfFindingTC/zygoticTranscripts_vectorized_noMultimappers/numberOfGenes_zygotic//",allRdata)
...
getGenes = function(allRdata_path,nMut){
  ...
  plot1 <- df %>%
    ...
    geom_line(aes(x = timepoints, y = series1), size = 1, alpha = 0.75,col='red') + theme_ameres(type = "barplot")  +  ylab(paste0("Number of CW with >", nMut, "TCs")) 
  ...
  plot2 <- df %>%
    ...
    geom_line(aes(x = timepoints, y = series2), size = 1, alpha = 0.75,col='blue') + theme_ameres(type = "barplot") + ylab(paste0("Number of MT CW with >", nMut, "TCs")) + ylim(c(0,15))
  
  grid.newpage()
  pdf(paste0("/Volumes/groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/plots/probabilityOfTC/NumberOfGenes_morethan",nMut,"TC_zygotic_noMultimappers.pdf"))
...
stochasticity = function(mutTake,nMut,refSample){
  ...
  colnames(allTPs) = c("TP1","TP2","TP3","TP4","TP5","TP6","TP7","TP8","TP9")
  ord <- hclust( dist((allTPs), method = "euclidean"), method = "ward.D" )$order
  allTPs = allTPs[ord,]
  #cluster_kmena = kmeans(allTPs,3)
  #allTPs$clus = cluster_kmena$cluster
  #allTPs = allTPs[order(allTPs$clus),]
  allTPs = melt(allTPs[,c(1:9)])
  ...
  p =  ggplot(allTPs,aes(x=variable,y = gene,fill=value)) + geom_tile() + scale_fill_gradientn(colours = c("white", "black"), values = c(0,0.1,1)) + ylab(paste0("genes with >=",nMut,"@ TP2")) + theme_ameres(type = "barplot")
  return(p)
}
...
for(ref in c(1:9)){
  ...
  pdf(paste0("/Volumes/groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/plots/probabilityOfTC/stochasticExpression_zygoticGenes_TP",ref,"reference_noMultimappers.pdf"))
