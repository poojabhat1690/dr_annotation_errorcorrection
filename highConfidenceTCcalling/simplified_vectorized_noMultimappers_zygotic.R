...
dir.create("////groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/probabilityOfFindingTC/zygoticTranscripts_vectorized_noMultimappers/") 
...
vectorizedsample = function(vcfFile,BamFile,countingWindows,barcode,condition){ 
  ...
  bamFile <- BamFile(file = BamFile)  #### assign and open bam file
  open(bamFile)
  params_neg <- ScanBamParam(which  = countingWindows_zygotic_minus, what = scanBamWhat(),tag=c("RA","MP"),flag =scanBamFlag(isMinusStrand =T) ) #### set the parameters required
  
  aln_minus <- scanBam(bamFile, param = params_neg) ### scan the bam
  ...
  aln_strand = as.data.frame(unlist(lapply(aln,function(x) as.character(x$strand))))
  ...
  alignmentMatrix = alignmentMatrix[-which(alignmentMatrix$mapq !=60),]
  ...
  write.table(finalMatrix,paste0("////groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/probabilityOfFindingTC/zygoticTranscripts_vectorized_noMultimappers/freqAllMutations_zygoticTranscripts",barcode,condition,".txt"),sep="\t",quote = F)
...
rpms_allCountingWindows_mt = rpms_allCountingWindows_mt %>% dplyr::select(1:6)
countingWindows_zygotic =countingWindows_zygotic %>%  dplyr::select(1:6)
...
for(i in 1:length(barcodesOfInterest)){
  ...
  vectorizedsample(vcfFile = vcfFile_ofInterest,BamFile = BamFile_ofInterest,countingWindows = countingWindows_zygotic,barcode = barcodesOfInterest[i],condition = conditionsOfInterest[i])
}
