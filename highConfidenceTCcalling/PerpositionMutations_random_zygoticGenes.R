...
countingWindows_zygotic = rbind.data.frame(countingWindows_zygotic,rpms_allCountingWindows_mt)
countingWindows_zygotic_gr = makeGRangesFromDataFrame(df = countingWindows_zygotic,keep.extra.columns = T,ignore.strand = F,seqnames.field = "V1",start.field = "V2",end.field = "V3",strand.field = "V6",starts.in.df.are.0based = T)
#sampleVec = c(1:nrow(rpms_allCountingWindows_Inj_TP9))


#### preapring the vcf file... 

vcf_R1 = read.table("//Volumes//groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/slamDunk_combinedStageSpecific/snp/combinedFile_CAAGCA.fastq.gz_adapterTrimmed_slamdunk_mapped_filtered_snp.vcf")
vcf_R2 = read.table("////Volumes/groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/slamDunk_combinedStageSpecific/snp/combinedFile_TAGGCT.fastq.gz_adapterTrimmed_slamdunk_mapped_filtered_snp.vcf")
vcf_R3 = read.table("///Volumes//groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/slamDunk_combinedStageSpecific/snp/combinedFile_ACGTCT.fastq.gz_adapterTrimmed_slamdunk_mapped_filtered_snp.vcf")
vcfFile = rbind(vcf_R1,vcf_R2,vcf_R3)
...
vectorizedsample = function(vcfFile,BamFile,countingWindows,barcode,condition){ 
  ...
  if(length(noMPtags)>0){
    for(MPtags in 1:length(noMPtags)){
      ...
      #aln[[noMPtags[MPtags]]]$tag$MP = rep(NA, 1) 
    }
  ...
  write.table(finalMatrix,paste0("///Volumes/groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/probabilityOfFindingTC/zygoticTranscripts_vectorized/randomProbabilities/onlyZygoticTranscripts",barcode,condition,".txt"),sep="\t",quote = F)
...
vectorizedsample(vcfFile = vcfFile,BamFile = "///Volumes/groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/slamDunk_combinedStageSpecific/filter/out.bam",countingWindows = countingWindows_zygotic_gr,barcode = "all",condition = "combinedBam")
