##### this is depreciated.... please check get error rates_perTimepoint.R
...
differentNumberOfTCreads = function(countingWindows_zygotic_gr,bamFile,vcfFIle,nMut){
  
  countingWindows_zygotic_plus = countingWindows_zygotic_gr %>% filter(V6 == "+")
  countingWindows_zygotic_minus = countingWindows_zygotic_gr %>% filter(V6 == "-")
  ...
  bamFile <- BamFile(bamFile)  #### assign and open bam file
  open(bamFile)
  
  params <- ScanBamParam(which  = countingWindows_zygotic_plus, what = scanBamWhat(),tag=c("RA","MP")) #### set the parameters required
  ...
  aln_mapq = as.data.frame(unlist(lapply(aln,function(x) as.data.frame(x$mapq))))
  alignmentMatrix = cbind.data.frame(aln_chr,aln_RA,aln_MP,aln_position,aln_strand,aln_phred,aln_mapq)
  colnames(alignmentMatrix) = c("chr","RA","MP","startPos","strand","qual","mapq")
  ...
  vcfFile_mutn = paste(vcfFile_data$V1,(vcfFile_data$V2+1),sep="_") ### the vcf file is 0 based... 
  ...
  finalMatrix = bind_rows(RAtags_final,readsWith0Mutations_subset)
   write.table(finalMatrix,paste0("///groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/errorRates_untreatedSamples_zebrafish/numberOfreadsWith",nMut,"_TC.txt"),quote = F,sep="\t")
...
#rpms_allCountingWindows = rpms_allCountingWindows_mt
untreatedSamples  =rpms_allCountingWindows %>% select(V1,V2,V3,V4,V5,V6,contains("Unt"))  %>% mutate(Means = rowSums(.[,-c(1:6)])) %>% filter(Means != 0 )
#countingWindows_zygotic_gr = makeGRangesFromDataFrame(df = untreatedSamples,keep.extra.columns = T,ignore.strand = F,seqnames.field = "V1",start.field = "V2",end.field = "V3",strand.field = "V6",starts.in.df.are.0based = T)
...
sampleInfo = sampleInfo[grep("Unt",sampleInfo$V3),]
vcfFile = paste0("combinedFile_",sampleInfo$V2,".fastq.gz_adapterTrimmed_slamdunk_mapped_filtered_snp.vcf")
vcfFilePath = paste0("/groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/slamDunk_combinedStageSpecific/snp/",vcfFile)
vcfFile_data = lapply(vcfFilePath,function(x) read.table(x))
vcfFile_data = do.call(rbind.data.frame,vcfFile_data)
vcfFile_data = vcfFile_data[!duplicated(vcfFile_data[,c("V1","V2")]),]

allBamFiles =list.files("//groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/errorRates_untreatedSamples_zebrafish/separateNumTCs/","*t_1.bam")
allBamFiles = allBamFiles[-grep(".bai",allBamFiles)]
allBamFiles = paste0("///groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/stageSpecific/outputs/errorRates_untreatedSamples_zebrafish/separateNumTCs/",allBamFiles)

for(i in 1:length(allBamFiles)){
differentNumberOfTCreads(countingWindows_zygotic_gr = untreatedSamples,bamFile = allBamFiles[i],vcfFIle = vcfFile_data,nMut = i)
}